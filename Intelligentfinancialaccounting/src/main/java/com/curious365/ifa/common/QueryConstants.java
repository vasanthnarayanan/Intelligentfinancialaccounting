...
public class QueryConstants {
	...
	public static final String INSERT_CUSTOMER = "insert into customer(customerid,name,tradename,activeflag,initialbalance,currentbalance,customeraddress,customerstate,customerphonenumber,taxuniqueid,priveleged) values(id_sequence.nextVal,?,?,?,?,?,?,?,?,?,?)";
	public static final String UPDATE_CUSTOMER = "update customer set name=?,tradename=?,initialbalance=?,customeraddress=?,customerstate=?,customerphonenumber=?,taxuniqueid=?,priveleged=? where customerid=?";
	...
	public static final String LIST_ITEMS_LIKE = "select item.*,type,hasdecimal,taxrate,quantity from item,item_type,item_quantity  where activeflag=? and item.typeid=item_type.typeid and item.quantityid=item_quantity.quantityid and itemname||itemid like ?";	
	 ...
	 * 
	 */
	public static final String LIST_INVOICE_LIKE = "select invoiceid,to_char(invoicedate,'DD/MM/YYYY')\"invoicedate\",invoicetype,invoicecustomerid,invoice.remarks as remarks,name as invoicecustomername,tradename from invoice inner join customer on invoice.invoicecustomerid=customer.customerid where invoice.activeflag=? and  customer.priveleged=0 and invoicetype=? and invoiceid like ?";
	public static final String LIST_INVOICE_INCL_PRIV_LIKE = "select invoiceid,to_char(invoicedate,'DD/MM/YYYY')\"invoicedate\",invoicetype,invoicecustomerid,invoice.remarks as remarks,name as invoicecustomername,tradename from invoice inner join customer on invoice.invoicecustomerid=customer.customerid where invoice.activeflag=? and invoicetype=? and invoiceid like ?";
	public static final String LIST_INVOICE_BY_MONTH = "select invoiceid,taxinvoiceid,to_char(invoicedate,'DD/MM/YYYY')\"invoicedate\",invoicetype,invoicecustomerid,invoice.remarks as remarks,name as invoicecustomername,tradename from invoice inner join customer on invoice.invoicecustomerid=customer.customerid where invoice.activeflag=? and  customer.priveleged=0 and invoice.invoicedate like ? order by invoicedate,invoiceid";
	public static final String LIST_INVOICE_INCL_PRIV_BY_MONTH = "select invoiceid,taxinvoiceid,to_char(invoicedate,'DD/MM/YYYY')\"invoicedate\",invoicetype,invoicecustomerid,invoice.remarks as remarks,name as invoicecustomername,tradename from invoice inner join customer on invoice.invoicecustomerid=customer.customerid where invoice.activeflag=? and invoice.invoicedate like ? order by invoicedate,invoiceid";
	public static final String GET_INVOICE_BY_ID = "select invoiceid,to_char(invoicedate,'DD/MM/YYYY')\"invoicedate\",invoicetype,invoicecustomerid,invoice.remarks as remarks,name as invoicecustomername,tradename from invoice inner join customer on invoice.invoicecustomerid=customer.customerid where invoice.activeflag=? and invoiceid=?";
	public static final String COUNT_INVOICE_BY_ID = "select count(1) from invoice where activeflag=? and invoiceid=?";
	public static final String CREATE_INVOICE = "insert into invoice(invoiceid,invoicedate,invoicetype,invoicecustomerid,invoicetransportid,orderid,remarks,taxinvoiceid,activeflag) values(invoice_id_sequence.nextVal,?,?,?,?,?,?,?,?)";
	...
	public static final String UPDATE_INVOICE_WT_TAX_INVOICE = "update invoice set taxinvoiceid=? where invoiceid=?";
	public static final String SOFT_DELETE_INVOICE  = "update invoice set activeflag=? where invoiceid=?";
	
	/**Tax Invoice
	 * 
	 */
	public static final String LIST_TAX_INVOICE_LIKE = "select taxinvoiceid,to_char(taxinvoicedate,'DD/MM/YYYY')\"taxinvoicedate\",taxinvoicetype,taxinvoicecustomerid,taxinvoice.remarks as remarks,name as taxinvoicecustomername,tradename as consignorname from taxinvoice inner join customer on taxinvoice.taxinvoicecustomerid=customer.customerid where taxinvoice.activeflag=? and  customer.priveleged=0 and taxinvoicetype=? and taxinvoiceid like ?";
	public static final String LIST_TAX_INVOICE_INCL_PRIV_LIKE = "select taxinvoiceid,to_char(taxinvoicedate,'DD/MM/YYYY')\"taxinvoicedate\",taxinvoicetype,taxinvoicecustomerid,taxinvoice.remarks as remarks,name as taxinvoicecustomername,tradename as consignorname from taxinvoice inner join customer on taxinvoice.taxinvoicecustomerid=customer.customerid where taxinvoice.activeflag=? and taxinvoicetype=? and taxinvoiceid like ?";
	public static final String LIST_TAX_INVOICE_BY_MONTH = "select taxinvoiceid,to_char(taxinvoicedate,'DD/MM/YYYY')\"taxinvoicedate\",taxinvoicetype,taxinvoicecustomerid,taxinvoice.remarks as remarks,name as taxinvoicecustomername,tradename as consignorname from taxinvoice inner join customer on taxinvoice.taxinvoicecustomerid=customer.customerid where taxinvoice.activeflag=? and  customer.priveleged=0 and taxinvoice.taxinvoicedate like ? order by taxinvoicedate,taxinvoiceid";
	public static final String LIST_TAX_INVOICE_INCL_PRIV_BY_MONTH = "select taxinvoiceid,to_char(taxinvoicedate,'DD/MM/YYYY')\"taxinvoicedate\",taxinvoicetype,taxinvoicecustomerid,taxinvoice.remarks as remarks,name as taxinvoicecustomername,tradename as consignorname from taxinvoice inner join customer on taxinvoice.taxinvoicecustomerid=customer.customerid where taxinvoice.activeflag=? and taxinvoice.taxinvoicedate like ? order by taxinvoicedate,taxinvoiceid";
	public static final String GET_TAX_INVOICE_BY_ID = "select taxinvoiceid,to_char(taxinvoicedate,'DD/MM/YYYY')\"taxinvoicedate\",taxinvoicetype,taxinvoicecustomerid,taxinvoice.remarks as remarks,name as taxinvoicecustomername,tradename as consignorname,customeraddress as consignorAddress,customerstate as consignorStateCode,gst_state.statename as consignorState,taxuniqueid as consignorTaxUniqueId from taxinvoice inner join customer on taxinvoice.taxinvoicecustomerid=customer.customerid left join gst_state on customer.customerstate=gst_state.statecode where taxinvoice.activeflag=? and taxinvoiceid=?";
	...
	public static final String GET_LAST_INVOICE_ID = "select max(taxinvoiceid) from taxinvoice where activeflag=?";
	public static final String GET_FIRST_INVOICE_ID_BY_DATE = "select min(taxinvoiceid) from taxinvoice where taxinvoicedate=? and activeflag=?";
	public static final String GET_LAST_INVOICE_ID_BY_DATE = "select max(taxinvoiceid) from taxinvoice where taxinvoicedate=? and activeflag=?";
	public static final String COUNT_TAX_INVOICE_BY_DATE = "select count(1) from taxinvoice where taxinvoicedate=? and activeflag=?";
	
	/**
	 * Audited Sales
	 */
	public static final String GET_AUDITED_SALES ="select auditedsalesid,salesrecordid,to_char(salesdate,'DD/MM/YYYY')\"salesdate\",salescustomerid,name as salescustomername,itemname as salesitemname,salesitemid,salespieces,salescost,salesremarks,quantity as salesitemquantity,type as salesitemtype,stock as salesstock,salestaxrate,salestax,invoiceid from auditedsales,customer,item,item_type,item_quantity where auditedsalesid=? and salescustomerid=customerid and salesitemid=itemid and item.quantityid = item_quantity.quantityid and item.typeid = item_type.typeid and auditedsales.activeflag=?";
	...
	public static final String UPDATE_AUDITED_SALES = "update auditedsales set salesdate=?,salescustomerid=?,salesitemid=?,salespieces=?,salescost=?,salesremarks=?,salestax=?,salestaxrate=? where auditedsalesid=?";
	public static final String SOFT_DELETE_AUDITED_SALES = "update auditedsales set activeflag=? where auditedsalesid=?";
	public static final String LIST_AUDITED_SALES_BY_TAX_INVOICE_ID= "select auditedsalesid,salesrecordid,to_char(salesdate,'DD/MM/YYYY')\"salesdate\",salescustomerid,name as salescustomername,itemname as salesitemname,salesitemid,salespieces,salescost,salesremarks,quantity as salesitemquantity,type as salesitemtype,hsncode,stock as salesstock,salestaxrate,salestax,invoiceid,taxinvoiceid from auditedsales,customer,item,item_type,item_quantity where salescustomerid=customerid and salesitemid=itemid and item.quantityid = item_quantity.quantityid and item.typeid = item_type.typeid and auditedsales.activeflag=? and auditedsales.taxinvoiceid=?";
	public static final String LIST_AUDITED_SALES_BY_MONTH_YEAR = "select auditedsalesid,salesrecordid,to_char(salesdate,'DD/MM/YYYY')\"salesdate\",salescustomerid,name as salescustomername,itemname as salesitemname,salesitemid,salespieces,salescost,salesremarks,quantity as salesitemquantity,type as salesitemtype,stock as salesstock,salestaxrate,salestax,invoiceid,taxinvoiceid from auditedsales,customer,item,item_type,item_quantity where salescustomerid=customerid and salesitemid=itemid and item.quantityid = item_quantity.quantityid and item.typeid = item_type.typeid and auditedsales.activeflag=? and auditedsales.salesdate like ? order by auditedsales.salesdate,auditedsales.auditedsalesid";
	public static final String LIST_AUDITED_SALES_BY_MONTH = "select auditedsalesid,salesrecordid,to_char(salesdate,'DD/MM/YYYY')\"salesdate\",salescustomerid,name as salescustomername,itemname as salesitemname,salesitemid,salespieces,salescost,salesremarks,quantity as salesitemquantity,type as salesitemtype,stock as salesstock,salestaxrate,salestax,invoiceid,taxinvoiceid from auditedsales,customer,item,item_type,item_quantity where salescustomerid=customerid and salesitemid=itemid and item.quantityid = item_quantity.quantityid and item.typeid = item_type.typeid and customer.priveleged=0 and auditedsales.activeflag=? and auditedsales.salesdate like ? order by auditedsales.salesdate,auditedsales.auditedsalesid";
	public static final String LIST_AUDITED_SALES_INCL_PRIV_BY_MONTH = "select auditedsalesid,salesrecordid,to_char(salesdate,'DD/MM/YYYY')\"salesdate\",salescustomerid,name as salescustomername,itemname as salesitemname,salesitemid,salespieces,salescost,salesremarks,quantity as salesitemquantity,type as salesitemtype,stock as salesstock,salestaxrate,salestax,invoiceid,taxinvoiceid from auditedsales,customer,item,item_type,item_quantity where salescustomerid=customerid and salesitemid=itemid and item.quantityid = item_quantity.quantityid and item.typeid = item_type.typeid and auditedsales.activeflag=? and auditedsales.salesdate like ?";
	...
	public static final String COUNT_FILTERED_SALES = "select count(1) from sales,customer,item,item_type,item_quantity where sales.activeflag=? and customer.priveleged=0 and sales.activeflag=customer.activeflag and salescustomerid=customerid and salesitemid=itemid and item.quantityid = item_quantity.quantityid and item.typeid = item_type.typeid";
	// sorted by date and id to ensure order is preserved when two records belong to same date
	public static final String LIST_FILTERED_SALES = "select * from ( select a.*, ROWNUM rnum from (select salesrecordid,to_char(salesdate,'DD/MM/YYYY')\"date\",salescustomerid,name,itemname as salesitemname,salesitemid,salespieces,salescost,salesremarks,quantity as salesitemquantity,type as salesitemtype from sales,customer,item,item_type,item_quantity where sales.activeflag=? and customer.priveleged=0 and sales.activeflag=customer.activeflag and salescustomerid=customerid and salesitemid=itemid and item.quantityid = item_quantity.quantityid and item.typeid = item_type.typeid order by salesdate,salesrecordid) a where ROWNUM <= ?)where rnum  > ?";
	public static final String COUNT_FILTERED_SALES_INCL_PRIV = "select count(1) from sales,customer,item,item_type,item_quantity where sales.activeflag=? and sales.activeflag=customer.activeflag and salescustomerid=customerid and salesitemid=itemid and item.quantityid = item_quantity.quantityid and item.typeid = item_type.typeid";
	public static final String LIST_FILTERED_SALES_INCL_PRIV = "select * from ( select a.*, ROWNUM rnum from (select salesrecordid,to_char(salesdate,'DD/MM/YYYY')\"date\",salescustomerid,name,itemname as salesitemname,salesitemid,salespieces,salescost,salesremarks,quantity as salesitemquantity,type as salesitemtype from sales,customer,item,item_type,item_quantity where sales.activeflag=? and sales.activeflag=customer.activeflag and salescustomerid=customerid and salesitemid=itemid and item.quantityid = item_quantity.quantityid and item.typeid = item_type.typeid order by salesdate,salesrecordid) a where ROWNUM <= ?)where rnum  > ?";
